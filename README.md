...
minecraft-web-GUI ist eine Webseite im Minecraft Design, die dafür ausgelegt ist eigene Minecraft Server (die auf Windows) laufen zu verwalten. Dafür muss auf dem Server auf dem die Daten dieses Reposetorys liegen kein Windows Server sein und auch nicht der gleiche Server sein.
...
Installiere zuerst auf deinem Windows-Server das Reposetory "server-for-minecraft-web-GUI" und folge auch dort der Installationsanleitung.

Empfohlen: bei Server mit GIT: Baue eine dauerhafte Pull Verbindung zwischen dem Reposetory und deinem Server auf.
Anders: bei Server ohne GIT: Lade dir das Reposetory als Zip Datei herunter.
Speichere das Reposetory so ab, dass die Datei index.php über z.B. https://expample.com/index.php aufrufbar ist.
Erstelle unter ./scr/ die Datei config.inc.php mit diesem Inhalt.
```
<?php
...
Im Stammverzeichnis des Reposetorys findest du die Datei "database.sql".
...
Es wurde keine Verbindung zwischen der IP-Adresse des Windows-Servers und der Dynamic-DNS hergestellt.
...
```
Der Server ist momentan offline. Er kann von einem Administratoren gestartet werden.
```
#### Mögliche Ursachen
...
```
#### Mögliche Ursachen
Du hast eine falsche E-Mail oder einfalsches Passwort eingegeben.
...
Du hast in der Datenbank das Passwort im Klartext gespeichert oder eine falsche hashmethode verwendet.
Lösung: Achte darauf, dass das Passwort in der passwort_hash() methode gespeichert werden muss. Ändere dies also gegebenfalls.

Es wurde keine richtige Verbindung zur Datenbank aufgebaut
Lösung: Vergewissere dich, dass du dich richtigen Zugangsdaten eingegeben hast und dass der Datenbankbenutzer alle notwendigen berechtigungen hat.
...
Lösung: starte den Server, wenn du unten auf "Server Starten" klickst. Achte dabei, dass du eine Angemessene MB-RAM Anzahl eingibst mit der der Minecraft-Server gestartet werden soll.
...
```
#### Mögliche Ursachen
Ajax ist noch nicht richtig kofiguriert.
...
* Grundkentnisse in PHP
* Grundkentnisse in Apache und generell Webentwicklung
* Grundkentnisse in Minecraft
